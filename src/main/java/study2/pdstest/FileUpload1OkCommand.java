...
public class FileUpload1OkCommand implements StudyInterface {
	...
	public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		/*
		 	...
		 	MultipartRequest(저장소명(request), "서버에 저장될 파일의 경로", "서버에 저장될 파일의 최대용량", "코드변환방식", 기타옵션(파일명중복방지))
		...
		*/
		...
		// 업로드된 파일의 정보를 추출해보자...
		String originalFileName  = multipartRequest.getOriginalFileName("fName"); //파일 업로드할 때 파일명
		...
		if(originalFileName!= null && !originalFileName.equals("")) {
			...
			request.setAttribute("msg", "파일이 업로드실패.");
		}
